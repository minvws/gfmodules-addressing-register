...
def run():
    ...
    default_endpoint = endpoint_service.add_one(
        name=fake.text(20), description=fake.text(50),
    ...
    for x in range(10):
        organization_model = OrganizationModel(
            ...
            name=fake.company(),
            description=fake.text(50),
            parent_organization_id=random.choice(org_ids) if fake.boolean(50) and len(org_ids) > 3 else None
        )
        org = organization_service.add_one(organization_model)
        org_ids.append(org.id)
        ura_number = org.ura_number

        endpoint = endpoint_service.add_one(name=fake.text(20), description=fake.text(50), address=fake.url(),
...
def mass_session_commit(endpoints_ids: List, org_ids: List) -> None:
    for x in range(len(endpoints_ids)):
        with db.get_db_session() as session:
            ...
            session.add(endpoint_environment.EndpointEnvironment(
                endpoint_id=endpoints_ids[x],
                environment_type=random.choice(list(get_codes(environment.Environment))),
            ))
            session.add(organization_type_association.OrganizationTypeAssociation(
                organization_id=org_ids[x],
                organization_type=random.choice(list(get_codes(organization_type.OrganizationType))),
            ))
            session.add(organization_contact.OrganizationContact(
                organization_id=org_ids[x],
                contact_type=random.choice(list(get_codes(contact_type.ContactType))),
            ))
            contact_point = ContactPoint(
                system_type=random.choice(list(get_codes(contact_point_system.ContactPointSystem))),
                use_type=random.choice(list(get_codes(contact_point_use.ContactPointUse))),
                value=fake.text(50),
                rank=fake.random_int(1, 5),
                period_start_date=fake.date_time_between(start_date=datetime(2020, 1, 1),
                                                         end_date=datetime(2022, 1, 1)),
                period_end_date=fake.date_time_between(start_date=datetime(2022, 1, 2), end_date=datetime(2024, 1, 1)),
            )
...
def generate_endpoint_contact_points(endpoints_ids: List) -> None:
    ...
    for x in range(len(endpoints_ids)):
        with db.get_db_session() as session:
            session.add(endpoint_contact_point.EndpointContactPoint(
                endpoint_id=endpoints_ids[x],
