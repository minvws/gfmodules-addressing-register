...
from app.db.models.addressing import Example


@repository(Example)
class AddressingRepository:
    def __init__(self, session: Session):
        self.session = session

    def find_example(
        self, prefix: str, offset: int = 0, limit: int = 25
    ) -> Tuple[int, ScalarResult[Example]]:
        """
        ...
        data_stmt = (
            select(Example)
            .where(Example.name.istartswith(prefix, autoescape=True))
            .offset(offset)
            .limit(limit)
        )
        count_stmt = select(func.count()).where(
            Example.name.istartswith(prefix, autoescape=True)
        )
