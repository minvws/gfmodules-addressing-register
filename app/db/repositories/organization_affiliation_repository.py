...
class OrganizationAffiliationRepository(RepositoryBase):
    ...
    def create(self, data: OrganizationAffiliationEntry) -> OrganizationAffiliationEntry:
        try:
            # Update the latest flag for the previous version
            (self.db_session.query(OrganizationAffiliationEntry)
                .filter(OrganizationAffiliationEntry.fhir_id == data.fhir_id,
    ...
    def find(
        ...
        if "id" in conditions and conditions["id"] is not None:
            ...
            filter_conditions.append(OrganizationAffiliationEntry.id == conditions["id"]) # type: ignore
        if "active" in conditions and conditions["active"] is not None:
            filter_conditions.append(OrganizationAffiliationEntry.data["active"].astext.cast(Boolean) == conditions["active"])
        if "date" in conditions and conditions["date"] is not None:
            filter_conditions.append(OrganizationAffiliationEntry.created_at >= conditions["date"]) # type: ignore
        if "identifier" in conditions and conditions["identifier"] is not None:
            filter_conditions.append(OrganizationAffiliationEntry.fhir_id == conditions["identifier"]) # type: ignore
        if "participating_organization" in conditions and conditions["participating_organization"] is not None:
            filter_conditions.append(
                OrganizationAffiliationEntry.data["participatingOrganization"]["reference"].astext == "Organization/" + str(conditions["participating_organization"])
            )
        if "primary_organization" in conditions and conditions["primary_organization"] is not None:
            filter_conditions.append(
                OrganizationAffiliationEntry.data["organization"]["reference"].astext == "Organization/" + str(conditions["primary_organization"])
            )
