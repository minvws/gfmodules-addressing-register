...
from sqlalchemy import select, Boolean, cast, TIMESTAMP
...
class OrganizationAffiliationRepository(RepositoryBase):
    def get_one(
        ...
        stmt = (
            ...
            .where(OrganizationAffiliation.latest.__eq__(True), OrganizationAffiliation.deleted.__eq__(False))
            .filter_by(**kwargs)
        )
    ...
    def find(
        ...
        **conditions: bool|str|UUID|dict[str, Any]|datetime|None,
        ...
        if "active" in conditions and conditions["active"] is not None:
            filter_conditions.append(OrganizationAffiliation.data["active"].astext.cast(Boolean) == conditions["active"])
        ...
        if "participating_organization" in conditions and conditions["participating_organization"] is not None:
            filter_conditions.append(
                OrganizationAffiliation.data["participatingOrganization"]["reference"].astext == "Organization/" + str(conditions["participating_organization"])
            )

        if "primary_organization" in conditions and conditions["primary_organization"] is not None:
            filter_conditions.append(
                OrganizationAffiliation.data["organization"]["reference"].astext == "Organization/" + str(conditions["primary_organization"])
            )
        ...
        if "sort_history" in conditions and conditions["sort_history"] is True:
            ...
            stmt = stmt.order_by(cast(OrganizationAffiliation.data['meta']['lastUpdated'].astext, TIMESTAMP(timezone=True)).desc(),
                                 OrganizationAffiliation.version.desc())

        if "since" in conditions:
            filter_conditions.append(
                cast(OrganizationAffiliation.data['meta']['lastUpdated'].astext, TIMESTAMP(timezone=True)) >= conditions["since"]
            )
    ...
    def update(
        self, organization_affiliation: OrganizationAffiliation, fhir_data: Dict[str, Any]
    ...
    def _update_entry_latest(self, organization_affiliation: OrganizationAffiliation) -> None:
        (
            ...
            .filter(OrganizationAffiliation.fhir_id == organization_affiliation.fhir_id, OrganizationAffiliation.latest)
