...
class OrganizationService:
    ...
    def add_one(self, organization: OrganizationModel) -> Organization:
        with self.database.get_db_session() as session:
            ...
            try:
                if organization_repository.get(ura_number=str(organization.ura_number)) is not None:
                    logging.error(f"Failed to add organization {organization}: Organization with URA number already exists")
    ...
    def get_many(self, active: bool|None, name: str|None, description: str|None, parent_organization_id: UUID|None) -> List[Organization]:
        with self.database.get_db_session() as session:
            organization_repository = session.get_repository(OrganizationsRepository)
            params = {
                "name": name,
                ...
                "parent_organization_id": str(parent_organization_id) if parent_organization_id is not None else None,
            }
    ...
    def update_one(self, ura_number: UraNumber, active: Optional[bool], name: str|None, description: str|None, parent_org: UUID|None) -> Organization:
        with self.database.get_db_session() as session:
            organization_repository = session.get_repository(OrganizationsRepository)
            entity = organization_repository.get(ura_number=str(ura_number))
            if entity is None:
