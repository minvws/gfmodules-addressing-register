...
def test_ura_auth_raises_exception_when_missing_cert_header(mocker: MockerFixture) -> None:
    request = mocker.MagicMock(spec=Request)
    request.headers = {
    }
...
def test_enforce_cert_newlines_with_headers(certificate_data: Tuple[str, str, str]) -> None:
...
def test_enforce_cert_newlines_without_headers(certificate_data: Tuple[str, str, str]) -> None:
