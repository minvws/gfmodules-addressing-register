...
from tests.utils import create_supplier, check_key_value


def test_add_supplier(sqlite_client: TestClient, supplier_endpoint: str,
                      override_ura: Generator[None, None, None]) -> None:
...
def test_add_supplier_should_fail_due_to_missing_certificates(sqlite_client: TestClient, supplier_endpoint: str,) -> None:
    """Test failure when adding a supplier due to missing certificates."""
    response = sqlite_client.post(supplier_endpoint, json={})
    assert response.status_code == 401


def test_add_supplier_should_fail_due_to_duplicate_endpoint(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    ...
    assert check_key_value(response.json(), "text", "Care provider already has an update supplier endpoint set up, cannot have more than one")

def test_add_supplier_should_fail_due_to_missing_or_invalid_supplier_data(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    """Test failure when adding supplier with missing or invalid data."""
    response = sqlite_client.post(supplier_endpoint, json={})
    assert response.status_code == 422

def test_get_supplier(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
...
def test_get_supplier_should_fail_due_to_missing_certificates(sqlite_client: TestClient, supplier_endpoint: str) -> None:
...
def test_get_supplier_should_fail_due_to_missing_ura_number(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    """Test failure when retrieving supplier due to missing URA number."""
    response = sqlite_client.post(f"{supplier_endpoint}/get-update-supplier")
    assert response.status_code == 422

def test_get_supplier_should_fail_due_to_invalid_ura_number(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    """Test failure when retrieving supplier due to invalid URA number."""
    response = sqlite_client.post(
        f"{supplier_endpoint}/get-update-supplier",
        json={"ura_number": "00000000"}
    )
    assert response.status_code == 404

def test_update_supplier(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
...
def test_update_supplier_should_fail_due_to_missing_certificates(sqlite_client: TestClient, supplier_endpoint: str) -> None:
    """Test failure when updating supplier due to missing certificates."""
    response = sqlite_client.patch(supplier_endpoint, json={})
    assert response.status_code == 401

def test_update_supplier_should_fail_due_to_unknown_ura_number(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
...
def test_update_supplier_should_fail_due_to_missing_or_invalid_supplier_data(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    """Test failure when updating supplier with missing or invalid data."""
    response = sqlite_client.patch(supplier_endpoint, json={})
    assert response.status_code == 422

def test_delete_supplier(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
    """Test deleting an existing supplier."""
    sqlite_client.post(supplier_endpoint, json=create_supplier().model_dump())
    response = sqlite_client.request('DELETE', supplier_endpoint, json={"ura_number": "12345678"})
    assert response.status_code == 204

def test_delete_supplier_should_fail_due_to_missing_certificates(sqlite_client: TestClient, supplier_endpoint: str) -> None:
    """Test failure when deleting supplier due to missing certificates."""
    response = sqlite_client.request('DELETE', supplier_endpoint, json={"ura_number": "12345678"})
    assert response.status_code == 401

def test_delete_supplier_should_fail_due_to_invalid_ura_number(sqlite_client: TestClient, supplier_endpoint: str, override_ura: Generator[None, None, None]) -> None:
