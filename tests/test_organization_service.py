...
class BaseTestSuite(unittest.TestCase):
    ...
    def add_organization(self,
                         ura_number: str = "00000000", active: bool = False, name: str = "test_name",
                         description: str = "test_dscr",
                         parent_organization_id: uuid.UUID | None = None) -> Organization:
        organization = OrganizationModel(
            ura_number=UraNumber(ura_number), active=active, name=name, description=description,
...
class TestMissingParentOrganization(BaseTestSuite):
    def test_add_organization_should_fail_when_parent_organization_does_not_exist(self) -> None:
    ...
    def test_update_should_fail_when_parent_organization_does_not_exist(self) -> None:
        ...
        with self.assertRaises(ResourceNotFoundException) as context:
            self.organization_service.update_one(ura_number=UraNumber(expected1.ura_number), parent_org=uuid.uuid4(),
                                                 active=True, name="abcdef", description="abcdef")
            assert context.msg == "Parent organization not found"
...
class TestGetManyOrganizations(BaseTestSuite):
    def test_get_many_should_succeed_with_proper_values(self) -> None:
        ...
        actual_list = self.organization_service.get_many(ura_number=expected_2.ura_number)
...
class TestUpdateOrganization(BaseTestSuite):
    ...
    def test_update_one_should_fail_due_to_no_resource(self) -> None:
        with self.assertRaises(ResourceNotFoundException):
            self.organization_service.update_one(ura_number=UraNumber("12345678"), name="Updated_name", parent_org=None,
                                                 active=False, description="Updated_descr")
